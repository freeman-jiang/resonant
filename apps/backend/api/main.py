...
async def recommend(userId: UUID) -> list[PageResponse]:
    ...
    liked_pages = db.get_page_stubs_by_id(
...
async def get_user_feed(userId: str) -> list[PageResponse]:
    ...
    messages_task = asyncio.create_task(_process_messages(messages))
    recommended_task = asyncio.create_task(recommend(userId=userId))

    result, _ = await messages_task
    recommended = await recommended_task

    # Interweave the two lists, alternating between recommended and messages
    result = [x for t in zip(result, recommended) for x in t]
